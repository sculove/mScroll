...
var _$ = function() {
    ...
    function _getTranslateOffsetFromStyle(e) {
        var top = 0, left = 0, temp = [], transValue = [], s = e.style[_.toPrefixStr("transform")];
        if (!!s && s.length > 0) {
            ...
            } else temp = s.match(/translate.{0,2}\((.*)\)/);
    ...
    var _ua = navigator.userAgent, _ = {
        ...
        cssPrefix: function() {
            ...
            if ("undefined" !== typeof bodyStyle.webkitTransition) return "webkit"; else if ("undefined" !== typeof bodyStyle.transition) return ""; else if ("undefined" !== typeof bodyStyle.MozTransition) return "Moz"; else if ("undefined" !== typeof bodyStyle.OTransition) return "O"; else if ("undefined" !== typeof bodyStyle.msTransition) return "ms";
        }(),
        extend: function(target, obj) {
            for (var p in obj) target[p] = obj[p];
            return target;
        },
        ...
        getTranslateOffset: function(e) {
            var offset;
            if (_.android && 3 === parseInt(_.version, 10)) offset = _getTranslateOffsetFromStyle(e); else if ("WebKitCSSMatrix" in window && "m11" in new WebKitCSSMatrix()) offset = _getTranslateOffsetFromCSSMatrix(e); else offset = _getTranslateOffsetFromStyle(e);
            return offset;
        },
        getStyleOffset: function(style) {
            var left = parseInt(style.left, 10), top = parseInt(style.top, 10);
            return {
    ...
    _.version = function() {
        ...
        if (_.ios) {
            ...
            if (null !== mv && mv.length > 1) version = mv[1];
        } else if (_.android) {
    ...
    _.use3d = function() {
        if (_.chromeBrowser && _.version < "25" && !_.sBrowser) return false;
        var result = false;
        if (_.ios || _.firefoxBrowser) result = true; else if (_.android) {
            if (_.version >= "4.1.0") if (/EK-GN120|SM-G386F/.test(_ua)) result = false; else result = true; else if (_.version >= "4.0.3" && /SHW-|SHV-|GT-|SCH-|SGH-|SPH-|LG-F160|LG-F100|LG-F180|LG-F200|EK-|IM-A|LG-F240|LG-F260/.test(_ua) && !/SHW-M420|SHW-M200|GT-S7562/.test(_ua)) result = true;
        } else result = true;
        return result;
    }();
    _.useTransition = function() {
        if (_.android) {
            if (_.version >= "4.3") return true;
        } else if (_.ios) if (parseInt(_.version, 10) <= "6") return true;
        return false;
    }();
    _.hasClickBug = function() {
        return _.ios;
    }();
    _.hasHighlightBug = function() {
        return _.chromeBrowser && !_.sBrowser && _.version < 35;
    }();
    _.hasOffsetBug = function() {
        var result = false;
        if (_.android) {
            if (_.chrome || _.firefox) result = false; else if (_.version < "3") result = true;
        } else result = false;
        return result;
    }();
...
mComponent.prototype = {
    ...
    on: function(type, fn) {
        ...
        if ("undefined" == typeof handler) handler = this._eventHandler[type] = [];
        handler.push(fn);
        return this;
    },
    off: function(type, fn) {
        if (this.has(type)) {
            ...
            if (index != -1) this._eventHandler[type].splice(index, 1);
        }
        return this;
    },
    trigger: function(type, param) {
        ...
        if ("undefined" == typeof param._extend) {
            param._extend = [];
            param.stop = function() {
                if (param._extend.length > 0) param._extend[param._extend.length - 1].cancel = true;
            };
        }
        param._extend.push({
            cancel: false
        });
        for (var i = 0, len = fns.length; i < len; i++) fns[i].call(this, param);
...
var mEffect = function(effect) {
    ...
    var number = /^(\-?[0-9\.]+)(%|\w+)?$/, RGB = /^rgb\(([0-9]+)\s?,\s?([0-9]+)\s?,\s?([0-9]+)\)$/i, RGBA = /^rgba\(([0-9]+)\s?,\s?([0-9]+)\s?,\s?([0-9]+),\s?([0-9\.]+)\)$/i, HSL = /^hsl\(([0-9\.]+)\s?,\s?([0-9\.]+)%\s?,\s?([0-9\.]+)%\)$/i, HSLA = /^hsla\(([0-9\.]+)\s?,\s?([0-9\.]+)%\s?,\s?([0-9\.]+)%,\s?([0-9\.]+)\)$/i, hex = /^#([0-9A-F]{2})([0-9A-F]{2})([0-9A-F]{2})$/i, color3to6 = /^#([0-9A-F])([0-9A-F])([0-9A-F])$/i;
    var getUnitAndValue = function(v) {
        ...
        if (number.test(v)) {
            ...
            value = {
                rgb: [ parseInt(RegExp.$1, 10), parseInt(RegExp.$2, 10), parseInt(RegExp.$3, 10), 1 ]
            };
            unit = "color";
        } else if (RGBA.test(v)) {
            value = {
                rgb: [ parseInt(RegExp.$1, 10), parseInt(RegExp.$2, 10), parseInt(RegExp.$3, 10), parseFloat(RegExp.$4) ]
            };
            unit = "color";
        } else if (HSL.test(v)) {
            value = {
                hsl: [ parseFloat(RegExp.$1), parseFloat(RegExp.$2) / 100, parseFloat(RegExp.$3) / 100, 1 ]
            };
            value.rgb = hsl2rgb.apply(this, value.hsl);
            unit = "color";
        } else if (HSLA.test(v)) {
            value = {
                hsl: [ parseFloat(RegExp.$1), parseFloat(RegExp.$2) / 100, parseFloat(RegExp.$3) / 100, parseFloat(RegExp.$4) ]
            };
            value.rgb = hsl2rgb.apply(this, value.hsl);
            unit = "color";
        } else if (hex.test(v = v.replace(color3to6, "#$1$1$2$2$3$3"))) {
            value = {
                rgb: [ parseInt(RegExp.$1, 16), parseInt(RegExp.$2, 16), parseInt(RegExp.$3, 16), 1 ]
            };
            unit = "color";
        } else {
            console.trace("error");
    ...
    var explode = function(str) {
        var ret = [];
        str.replace(/([^\s]+\([^\)]*\)|[^\s]+)\s?/g, function(_, a) {
            ret.push(a);
        });
        return ret;
    };
    var getUnitAndValueList = function(v) {
        var list = explode(v ? v + "" : "0"), ret = [];
        for (var i = 0, len = list.length; i < len; i++) ret.push(getUnitAndValue(list[i]));
        return ret;
    };
    var hsl2rgb = function(H, S, L, alpha) {
        ...
        var m = L - C / 2;
        return [ Math.round(255 * (R1 + m)), Math.round(255 * (G1 + m)), Math.round(255 * (B1 + m)), alpha ];
    };
    return function(start, end) {
            ...
            if (changed) {
                ...
                if (arrayStart.length !== arrayEnd.length && len > 1) {
                    switch (arrayStart.length) {
                      case 1:
                        _$.extend(arrayStart[1], arrayStart[0]);

                      case 2:
                        _$.extend(arrayStart[2], arrayStart[0]);

                      case 3:
                        _$.extend(arrayStart[3], arrayStart[1]);
                    }
                    switch (arrayEnd.length) {
                      case 1:
                        _$.extend(arrayEnd[1], arrayEnd[0]);

                      case 2:
                        _$.extend(arrayEnd[2], arrayEnd[0]);

                      case 3:
                        _$.extend(arrayEnd[3], arrayEnd[1]);
                    }
                }
                for (var i = 0, oStart, oEnd; i < len; i++) {
                    oStart = arrayStart[i];
                    oEnd = arrayEnd[i];
                    if (0 === oStart.value) oStart.unit = oEnd.unit; else if (0 === oEnd.value) oEnd.unit = oStart.unit;
                    if (oStart.unit != oEnd.unit) throw new Error("unit error (" + start + " ~ " + end + ")");
                }
            }
        };
        var instance = function(p) {
            ...
            for (var i = 0, len = Math.max(arrayStart.length, arrayEnd.length), oStart, oEnd, alpha, unit; i < len; i++) {
                ...
                unit = oStart.unit;
                var value = effect(p), getResult = function(s, d, unit) {
                    return Math.round(1e6 * ((d - s) * value + s)) / 1e6 + (unit || 0);
                };
                if ("color" !== unit) {
                ...
                if (start.hsl && end.hsl) {
                    ...
                    var s = 100 * Math.max(0, Math.min(1, getResult(start[1], end[1])));
                    var l = 100 * Math.max(0, Math.min(1, getResult(start[2], end[2])));
                    alpha = getResult(start[3], end[3]);
                    if (1 === alpha) values.push("hsl(" + [ h, s + "%", l + "%" ].join(",") + ")"); else values.push("hsla(" + [ h, s + "%", l + "%", alpha ].join(",") + ")");
                } else {
                    start = start.rgb;
                    end = end.rgb;
                    ...
                    } else values.push("rgba(" + [ r, g, b, alpha ].join(",") + ")");
                }
            }
            return values.join(" ");
        };
...
mEffect.linear.toString = function() {
    return "linear";
};
...
mEffect.easeIn.toString = function() {
    return "ease-in";
};
...
mEffect.easeOut.toString = function() {
    return "ease-out";
};

mEffect.easeInOut = mEffect(function(s) {
    return s < .5 ? .5 * mEffect.easeIn(0, 1)(2 * s) : .5 * mEffect.easeOut(0, 1)(2 * s - 1) + .5;
});

mEffect.easeInOut.toString = function() {
    return "ease-in-out";
};
...
mTouch.prototype = {
    ...
    _setSlope: function() {
        this.slope = {
            h: 1 * (window.innerHeight / 2 / window.innerWidth).toFixed(2),
            v: 1 * (window.innerHeight / (window.innerWidth / 2)).toFixed(2)
        };
    },
    _manageEvent: function(remove) {
        ...
        null != this._eventType.cancel && this.wrapper[method](this._eventType.cancel, this);
    },
    ...
    _getTouchInfo: function(e) {
        var result = [], time = e.timeStamp;
        if ("touch" == this._eventType.type) {
            var touches = e.touches;
            if (touches.length <= 1) touches = e.changedTouches;
    ...
    _resetTouchInfo: function() {
        if (0 == this.info["actions"].length) return;
        this.info = {
            start: {},
            before: {},
            toucheCount: 0,
    ...
    _determineMoveType: function(info) {
        if (this.moveType == mTouch.NONE || this.moveType == mTouch.TAP) {
            var distX = Math.abs(this.info["start"].x - info.x), distY = Math.abs(this.info["start"].y - info.y), dist = distX + distY;
            if (distX <= this.option.tapThreshold && distY <= this.option.tapThreshold) this.moveType = mTouch.TAP; else this.moveType = mTouch.NONE;
            if (this.option.slopeThreshold <= dist) {
                var nSlope = parseFloat((distY / distX).toFixed(2), 10);
                if (this.slope.h === -1 && this.slope.v === -1) this.moveType = mTouch.DSCROLL; else if (nSlope <= this.slope.h) this.moveType = mTouch.HSCROLL; else if (nSlope >= this.slope.v) this.moveType = mTouch.VSCROLL; else this.moveType = mTouch.DSCROLL;
            }
        }
    },
    _getEventParam: function(info) {
        var startInfo = this.info["start"], beforeInfo = this.info["before"], shortestInfo = this.info["actions"][0], ret = {
    ...
    _start: function(e) {
        ...
        if (infos.length > 1) return; else this._resetTouchInfo();
    ...
    _move: function(e) {
        ...
        if (this.info["toucheCount"] <= 0) return;
        var info = this._getTouchInfo(e)[0];
        this._determineMoveType(info);
        var param = this._getEventParam(info), dist = 0;
        ...
        if (dist < this.option.moveThreshold || dist < this.option.slopeThreshold || this.moveType == mTouch.NONE || this.moveType == mTouch.TAP) return;
...
function mScroll(el, option) {
    this.option = {
        ...
        effect: mEffect.cubicBezier(.18, .35, .56, 1),
        useHorizontal: false,
        useVertical: true,
...
mScroll.prototype = {
    ...
    _setInitStyle: function(css) {
        ...
        if (this.option.useTransition) css[_$.toPrefixStr("transitionTimingFunction")] = this.option.effect.toString();
        return css;
    },
    _setStyle: function() {
        var wrapperStyle = this.wrapper.style, wrapperCss = {
            overflow: "hidden"
        }, scrollerCss = {
            left: "0px",
            top: "0px"
        };
        ("" == wrapperStyle.position || "static" == wrapperStyle.position) && (wrapperCss.position = "relative");
        this.scroller = this.wrapper.children[0];
        this.scrollerStyle = this.scroller.style;
        ("" == this.scrollerStyle.position || "static" == this.scrollerStyle.position) && (scrollerCss.position = "relative");
    ...
    _manageEvent: function(remove) {
        ...
        !this.option.useTransition && (this.updater = function() {
            if (this._bufferPos.x != this.x || this._bufferPos.y != this.y) this._setPos(this._bufferPos.x, this._bufferPos.y);
            this._startUpdater();
        }.bind(this));
        _$.hasOffsetBug && (this._fixOffsetBugFunc = function() {
            if (this.scroller) {
                this._translateToStyle(this.scroller);
                if (parseInt(_$.version, 10) <= 3) this.dummy.focus();
    ...
    _createScrollbar: function(direction) {
        var className = "v" == direction ? mScroll.VSCROLLBAR_CLASS : mScroll.HSCROLLBAR_CLASS, scrollbar = this.wrapper.querySelector("div." + className), indicator;
        if (!scrollbar) {
            var scrollbarStyle = "pointer-events:none; position:absolute; z-index:9999;", indecatorStyle = "position:absolute;background-color:rgba(0,0,0,0.3);border-radius:3px;left:0px;top:0px;";
            indicator = document.createElement("div"), scrollbar = document.createElement("div");
    ...
    _refreshScrollbar: function(direction) {
        ...
        } else if (!this.useHScroll || this.wrapperW == this.scrollerW) {
    ...
    _preventSystemScroll: function(e) {
        switch (e.moveType) {
          case mTouch.HSCROLL:
            if (this.useHScroll) if (!this.option.useBounce && (this.x >= 0 && e.vectorX > 0 || this.x <= this.maxScrollX && e.vectorX < 0)) return; else {
                e.preventDefault();
                e.stopPropagation();
            }
            break;
          ...
          case mTouch.VSCROLL:
            if (this.useVScroll) if (!this.option.useBounce && (this.y >= 0 && e.vectorY > 0 || this.y <= this.maxScrollY && e.vectorY < 0)) return; else {
                e.preventDefault();
                e.stopPropagation();
            }
            break;
    ...
    _move: function(e) {
        ...
        if (this.trigger("beforeMove", e)) {
            ...
            if (this.option.useBounce) {
                ...
                if (e._type.indexOf("touch") != -1) this._timer["touch"] = setTimeout(function() {}.bind(this), 500);
            } else {
    ...
    _setPos: function(x, y) {
        ...
        var styleOffset = _$.hasOffsetBug ? _$.getStyleOffset(this.scrollerStyle) : {
            left: 0,
            top: 0
        };
        x -= styleOffset.left, y -= styleOffset.top;
    ...
    _setScrollbarPos: function(direction, pos) {
        ...
        var Indicator = this[direction + "indicator"], IndicatorStyle = Indicator.style;
        if (_$.hasOffsetBug) {
            var bufferPos = parseInt("h" === direction ? IndicatorStyle.left : IndicatorStyle.top, 10);
            pos -= isNaN(bufferPos) ? 0 : bufferPos;
        }
        IndicatorStyle[_$.toPrefixStr("transform")] = _$.getTranslate("h" == direction ? pos + "px" : 0, "h" == direction ? 0 : pos + "px", this.option.use3d);
    },
    ...
    _endForScroll: function(e) {
        ...
        } else param.duration = 0;
        if (this.trigger("beforeScroll", param)) if (isMomentum) if (this.option.useBounce) this.scrollTo(param.nextX, param.nextY, param.duration); else this.scrollTo(this._boundaryX(param.nextX), this._boundaryY(param.nextY), param.duration); else if (this.useHScroll && (param.nextX > 0 || param.nextX < this.maxScrollX) || this.useVScroll && (param.nextY > 0 || param.nextY < this.maxScrollY)) this.restore(300); else this._setPos(param.nextX, param.nextY);
    },
    restore: function(duration) {
        var nextX = this._boundaryX(this.x), nextY = this._boundaryY(this.y);
        if (nextX === this.x && nextY == this.y) this.isPlaying = false; else this.scrollTo(nextX, nextY, duration);
    },
    _stop: function() {
        if (this.option.useTransition) ; else {
    ...
    _animate: function() {
        ...
        var self = this;
        this.isPlaying = true;
        if (this.option.useTransition) ; else this._animationTimer(step);
    },
    _animationTimer: function(step) {
        var self = this;
        var startTime = new Date().getTime(), fx = this.useHScroll ? this.option.effect(this.x, step.x) : 0, fy = this.useVScroll ? this.option.effect(this.y, step.y) : 0, now;
        this._startUpdater();
        !function animate() {
            ...
            if (self.isPlaying) self._timer["ani"] = requestAnimationFrame(animate); else self._stopUpdater();
        }();
    },
    ...
    _getMomentum: function(distance, speed, size, current, max) {
        var deceleration = this.option.deceleration, momentum = speed * speed / 2, nextDist = momentum / deceleration, nextDuration = 0, outBoundSize = 0;
        if (distance < 0 && nextDist > current) {
            outBoundSize = size / (6 / (nextDist / speed * deceleration));
            current += outBoundSize;
